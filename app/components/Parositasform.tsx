...
export default function ParositasForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Összekészítjük az adatokat, amiket átküldünk a chat felületnek
    ...
    // HA BE VAN JELENTKEZVE: Először elmentjük a profilját az adatbázisba
    if (session?.user?.email) {
      ...
      // HA VENDÉG KÉNT VAN ITT (Nincs bejelentkezve): 
      // Nem mentünk adatbázisba, csak egyből átirányítjuk a chatbe!
      router.push(`/dashboard?${params.toString()}`);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <p className="text-[10px] text-gray-500 text-center -mt-3">
        {session 
          ? "A gomb megnyomásával frissítjük a profilodat és indul a keresés." 
          : "Vendégként használod az oldalt. A keresés azonnal indul."}
      </p>
    </form>
  );
}