        ...
        <div className="flex items-center gap-4">
          ...
          {status === "loading" ? (
            ...
            <div className="flex items-center gap-3">
              {session.user?.image && (
                <img src={session.user.image} alt="Profil" className="w-8 h-8 rounded-full border border-white/20" />
              )}
              <button
                onClick={() => signOut()}
                className="hidden sm:inline-flex text-xs font-medium text-gray-400 hover:text-white transition"
              >
                Kijelentkezés
              </button>
            </div>
            ...
            <button
              ...
              className="hidden sm:inline-flex items-center gap-2 px-4 py-2 text-xs font-bold rounded-lg bg-white text-gray-900 hover:bg-gray-100 shadow-md transition active:scale-95"
            >
